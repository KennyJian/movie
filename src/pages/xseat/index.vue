...
export default {
  ...
  head() {
    return {
      title: "选座购票",
      meta: [{ hid: "选座购票", name: "选座购票", content: "选座购票" }]
    };
  },
  components: {
    Hall,
  ...
  methods: {
    ...
    getFieldInfo() {
      ...
      getData(
        process.env.baseUrl + "/cinema/getFieldInfo",
        "post",
        params
      ).then(
        res => {
            ...
            _this.readTextFile("/json/4dx.json", function(text) {
              ...
              if (res.data.hallInfo.seatFile) {
                ...
                if (res.data.hallInfo.seatFile.single) {
                  _this.$store.state.seatInfo.seatCharts.single =
                    res.data.hallInfo.seatFile.single;
                  for (let i = 0, len = soldSeats.length; i < len; i++) {
                    for (
                      let j = 0,
                        len1 = res.data.hallInfo.seatFile.single.length;
                      j < len1;
                      j++
                    ) {
                      for (
                        let k = 0,
                          len2 = res.data.hallInfo.seatFile.single[j].length;
                        k < len2;
                        k++
                      ) {
                        if (
                          soldSeats[i] ==
                          res.data.hallInfo.seatFile.single[j][k].seatId
                        ) {
                ...
                if (res.data.hallInfo.seatFile.couple) {
                  _this.$store.state.seatInfo.seatCharts.couple =
                    res.data.hallInfo.seatFile.couple;
                  for (let i = 0, len = soldSeats.length; i < len; i++) {
                    for (
                      let j = 0,
                        len1 = res.data.hallInfo.seatFile.couple.length;
                      j < len1;
                      j++
                    ) {
                      for (
                        let k = 0,
                          len2 = res.data.hallInfo.seatFile.couple[j].length;
                        k < len2;
                        k++
                      ) {
                        if (
                          soldSeats[i] ==
                          res.data.hallInfo.seatFile.couple[j][k].seatId
                        ) {
    ...
    confirmXseat() {
      ...
      getData(process.env.baseUrl + "/order/buyTickets", "post", params).then(
        res => {
          if (!res) return;
          if (res.status == 0) {
